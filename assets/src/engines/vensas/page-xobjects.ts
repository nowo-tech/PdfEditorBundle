...
export function removeDetectedWatermarks(page: PDFPage): number {
  const draws = pageObjectDraws(page);
  const formDraws = draws.filter((d) => d.type === 'form');
  if (formDraws.length === 0) return 0;
  const targets = formDraws.map((d) => d.rect);
  return removePageObjects(page, targets);
}
