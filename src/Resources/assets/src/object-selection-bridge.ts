...
/** Switch to annotate mode with no drawing tool so clicks select objects. */
export function activateObjectSelectMode(registry: EditorRegistry, documentId: string): void {
  const commands = plugin<CommandsCapability>(registry, 'commands');
  const annotation = plugin<AnnotationCapability>(registry, 'annotation');
  const form = plugin<FormCapability>(registry, 'form');

  commands?.execute('mode:annotate', documentId, 'api');
